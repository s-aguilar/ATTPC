...
import plotly
plotly.tools.set_credentials_file(username='s.aguilar', api_key='9nsa8bYdnLe430YmZVEx')
...
with open('geanttrialExtract-li8-alpha-n-noExE.dat') as f:
    l = json.load(f)
    info = []
    for d in l:
        x = d['data']
...
# Assume the left most point belongs to the incoming beam 10C. Sorting makes
# the left most point become the second tuple in the list for each event.
#
# Tuples are of form:
...
for index in range(len(x)):
    ...
    eloss10C= splineBeam.integrate(0,sortedFinal[3][2])     ##
    eloss4He = splineAlpha.integrate(0,sortedFinal[2][2])
    elossBeam = splineBeam.integrate(0,sortedFinal[1][2])   ##

    sortedFinal[0] = sortedFinal[0] + (eVert,)
    sortedFinal[3] = sortedFinal[3] + (eloss10C,)           ##
    sortedFinal[2] = sortedFinal[2] + (eloss4He,)
    sortedFinal[1] = sortedFinal[1] + (elossBeam,)          ##
...
x1_val = [x[3][0] for x in kinematicInfo]
x2_val = [x[2][0] for x in kinematicInfo]
xbeam_val = [x[1][0] for x in kinematicInfo]

yv_val = [x[0][1] for x in kinematicInfo]
y1_val = [x[3][1] for x in kinematicInfo]
y2_val = [x[2][1] for x in kinematicInfo]
ybeam_val = [x[1][1] for x in kinematicInfo]

rv_val = [x[0][2] for x in kinematicInfo]
r1_val = [x[3][2] for x in kinematicInfo]
r2_val = [x[2][2] for x in kinematicInfo]
rbeam_val = [x[1][2] for x in kinematicInfo]

evLoss_val = [x[0][3] for x in kinematicInfo]
e1Loss_val = [x[3][3] for x in kinematicInfo]
e2Loss_val = [x[2][3] for x in kinematicInfo]
ebeamLoss_val = [x[1][3] for x in kinematicInfo]
...
points2D = np.vstack([r1_plot,r2_plot]).T
tri = Delaunay(points2D)
simplices = tri.simplices

fig1 = FF.create_trisurf(x=r1_plot, y=r2_plot, z=ang2_plot,
                         simplices=simplices,
                         title="Test", aspectratio=dict(x=1, y=1, z=0.3))
...
# ax.plot_trisurf(r1_plot,r2_plot,ang1_plot,color='b',alpha=0.8,antialiased=True,edgecolor='none')#,label=r'$^{10}C(\alpha,\alpha)^{10}C$')
...
ax.scatter(l1,l2,phi1,color='r', s=4, alpha = 0.5,rasterized=True,label='Simulated')
# ax.set_xlim(0,50)
# ax.set_ylim(0,50)
# ax.set_zlim(0,90)
